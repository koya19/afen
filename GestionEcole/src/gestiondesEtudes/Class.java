	...
	public Module choisirModule(int a) {
	 ...
	 public void affichestudClass() {
		 if(this.stud.isEmpty()) {
			 System.err.println("Pas d'étudiants pour le moment");
		 }
		 else {
						 ...
						 for (Element e:m.getEleModule()) {
							 if(i==elementchoisi) {
								 if (e.getTotalheure()==0) {
									System.err.println("les heures de cet élément sont épuisées"); 
									r=2;
									break;
								 }
								 else {
								 ...
								 System.out.println("--Entrer le jour de la semaine\n    1-Lun\n    2-Mar\n    3-Mer\n    4-Jeu\n    5-Ven\n    6-Sam");
								 int jour=sc.nextInt();
								 System.out.println("--Entrer la période \n    1-Matin\n    2-Après-midi");
								 int periode=sc.nextInt();
								 System.out.println("choisir la dicipline\n    1-CM\n    2-TP\n    3-TD\n    4-AP");
								 int disciplinechoisie=sc.nextInt();
								 String discipline=new String();
								 if (disciplinechoisie==1) {
										discipline= "CM";
								 ...
								 edt.addElement(jour, periode, e, discipline);
								 System.out.println("--Choisir votre prochaine action:\n    1-ajouter un autre element du "+m.toString()+"\n    2-ajouter un element d'un autre module \n    3-retour");
								 r=sc.nextInt();
								 if (r==3) {
				 ...
				 for (Element e: m.getEleModule()) {
					 if (i==elementchoisi) {
						 ...
						 for (Student s:this.stud) {
								 ...
								 System.out.println("entrer la note de l'element de l'etudiant "+ s.toString());
								 int note=sc.nextInt();
								 sc.nextLine();
								 e.modnote(s,note);
	 ...
	 public void addAbs() throws ParseException {
		 System.out.println("Veuillez entrez la date du jour sous forme dd MM yyyy");
		 String d=sc.next();
		 Date date=s.parse(d);
		 for(EDT edt:this.emploi) {
			
			 if ((date.compareTo(edt.getBegin())>=0 && date.compareTo(edt.getEnd())<=0)==false) {
				 System.out.println("la date est introuvable");
			 }
			 else {
				 int i=0;
				 while(i==0) {
					 
				 System.out.println("choisir module");
				 this.affichermoduleClass();
				 try {
				 int modulechoisi=sc.nextInt();
				 if(1>modulechoisi|| modulechoisi>this.moduleClass.size()) {
				 ...
				 System.out.println("choisir etudiant");
				 this.affichestudClass();
				 int studchoisi=sc.nextInt();
		 ...
		 for(EDT edt:this.emploi) {
			 ...
			 if ((date.compareTo(edt.getBegin())>=0 && date.compareTo(edt.getEnd())<=0)==false) {
				...
				for(Absence abs: this.abs) {
					...
					System.out.println("cet fiche d'absence est introuvable");
				}
				 }catch(InputMismatchException e) {
						System.out.println();
						System.err.println("Ce choix est invalide");
